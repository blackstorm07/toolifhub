...
export default function PostOfficeFinder() {
  ...
  const handleSearch = async (e) => {
    ...
    setError('');
    setResults(null);
    trackToolUsage('post-office-finder', 'Post Office Finder');
    try {
      const res = await fetch(`/api/government/postoffice?q=${encodeURIComponent(query.trim())}`);
    ...
    <div className="space-y-5">
      <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-3">
        <input
          ...
          placeholder="Search by city, district, or post office name (e.g. Andheri)"
      ...
      {results && (
        <div className="space-y-3">
          <p className="text-sm text-muted-foreground">{results.length} post office{results.length > 1 ? 's' : ''} found</p>
          {results.map((po, i) => (
            <div key={i} className="rounded-xl border border-border p-4 sm:p-5">
              <div className="flex items-start justify-between gap-3 mb-2">
                ...
                <span className="font-mono text-sm bg-muted px-2 py-0.5 rounded-md flex-shrink-0">{po.pincode}</span>
              </div>
      ...
      {!results && !error && (
        <p className="text-sm text-muted-foreground text-center py-8">
          Search by city, district, or post office name to find all matching post offices in India.
        </p>
      )}
    </div>
  );
